...
<template>
  <section class="calc">
    ...
    <div class="calc-fields">
      ...
      <label v-for="attribute in fields" :key="attribute.id" class="field"><span>{{ attribute.name }}</span><select v-if="attribute.kind === 'select'" :value="calculator.chosenValue(attribute.id)" @change="calculator.chooseValue(attribute.id, typed($event))"><option v-if="calculator.chosenValue(attribute.id) === ''" value="" disabled>Как в товаре</option><option v-for="value in attribute.values.filter((value) => value.id !== null)" :key="value.id" :value="value.id">{{ value.name }}</option></select><input v-else :value="calculator.chosenQuantity(attribute.id)" :class="{ invalid: calculator.isInvalid(attribute.id) }" type="text" inputmode="decimal" @change="calculator.setQuantity(attribute.id, typed($event))" /></label>
    </div>
