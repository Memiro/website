...
5. Overrides разрешаются самим атрибутом (`Attribute.configure`) по справочнику
   категории и заменяют объявленное товаром значение, а не заводят настройку,
   которой у товара нет — иначе `ATTRIBUTE_VALUE_NOT_FOUND` (404); правило
   здесь одно с добавлением варианта и с калькулятором (ADR-0007).
