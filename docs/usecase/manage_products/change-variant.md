...
| `data` | `ChangeVariantForm` | Новые размер, overrides, порядок и, если владелец её вписал, цена (`manual_price`) |
...
6. Конфигурация полностью считается **или** цена вписана владельцем — иначе
...
9. Цена пересчитывается тем же владельческим вопросом — а вписанную владельцем
   берёт как есть и помечает вариант `price_is_manual` (ADR-0017). Правка без
   вписанной цены возвращает вариант на расчёт. Вариант заменяется,
