...
размер. Цена нового ребёнка считается заново — а вписанную владельцем копия
наследует вместе с признаком: пересчитать её не из чего (ADR-0017).
...
| `variant_id` | `VariantId` | Копируемый вариант |
| `data` | `DuplicateVariantWithSizeForm` | Новые ширина и высота |

## Output

| Поле | Тип | Смысл |
|---|---|---|
| `id` | `VariantId` | Идентификатор нового варианта |
...
5. Конфигурация с прежними overrides и новым размером полностью считается —
   иначе `INVALID_VARIANT_CONFIGURATION` (400). Копию варианта с вписанной
   ценой это правило не касается: считать её не из чего, и она несёт цену
   исходного (ADR-0017).
6. Варианта с новым повёрнутым размером и теми же итоговыми значениями ещё нет
   — иначе `DUPLICATE_VARIANT` (409).
7. Цена нового варианта считается владельческим вопросом без покупательских
   гейтов, но с наценкой за размер — либо наследуется вписанной вместе с
   `price_is_manual`; исходный вариант не меняется. Ошибки нет.
