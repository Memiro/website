...
class AttributeValue(models.Model):
    ...
    def is_charged(self) -> bool:
        ...
        Условие то же, по которому движок заводит статью: расходуемая
        единица и ставка при ней. Половина тарифа статьи не даёт, а
        полагаться на то, что `clean()` половин не пропустит, здесь
        нельзя — переносы и пачечные правки ходят мимо неё.
        Бесплатное значение (цвет рамы, холодный свет) описывает
        изделие и денег не стоит.
        """
        return self.unit in pricing.CHARGED_UNITS and self.rate is not None
    ...
    def full_label(self) -> str:
        ...
        разложения цены, и в подписи доплаты, которую эндпоинт расчёта
        отдаёт покупателю.
        """
        return f"{self.attribute.name}: {self.value}"

    def clean(self) -> None:
        """Три правила строки справочника, и все три — о молчаливой цене.

        Отсутствие признака денег не стоит; у числового атрибута строка
        одна, иначе неизвестно, чей тариф умножать на количество; тариф
        же — пара, и половина ставки молча считалась бы нулём.
        """
...
class MaterialPrice(AttributeValue):
    """Те же строки справочника, другой вход: экран «Материалы и цены».

    Цена материала живёт у значения атрибута (ADR-0007), и в разделе
    «Атрибуты» она лежит на третьем уровне вложенности: атрибут →
    значения → строка. Владелец, искавший там цену полотна за м², её
    не нашёл (тикет 17).

    Прокси, а не отдельная модель «Материал»: своей таблицы и своих
    полей у неё нет, поэтому и второй правды о цене она завести не
    может — подсветка остаётся одной строкой, у которой один тариф.
    """
...
class Product(models.Model):
    ...
    # Цену расчёта владелец гасит сам — там, где тарифам на этом
    # изделии не верит. Гаснет только число: поля размеров и меняемых
    # атрибутов остаются на карточке, и настроенное покупателем уезжает
    # менеджеру позицией заявки (ADR-0008). Имя поля названо по тому,
    # что признак делает с ценой, а не с калькулятором: «выключить
    # калькулятор» читалось бы не так, как он себя ведёт.
    # Предпосчитанных вариантов признак не касается — их цены заводит
    # владелец, и «от X ₽» каталога остаётся на месте
    hides_calculated_price = models.BooleanField(
        "не показывать цену расчёта",
        default=False,
        help_text=(
            "Конструктор на карточке остаётся: покупатель выбирает "
            "размер и материалы, а цену вместо сайта называет менеджер"
        ),
    )
...
# Посадочная сужает категорию одним-двумя атрибутами (ADR-0003):
# длинные хвосты фильтров индексировать незачем. Значений у атрибута
# при этом бывает несколько — они объединяются по ИЛИ и хвоста не
# удлиняют: «в раме» это по-прежнему один вопрос к товару
...
def exhausts_dictionary(attribute: Attribute, values: set[object]) -> bool:
    """Перечислены ли у атрибута все значения — сужения тогда нет.

    Значения одного атрибута объединяются по ИЛИ, и, назвав их все,
    посадочная спрашивает у товара то, на что тот отвечает всегда:
    страница становится дублем категории под своим адресом — ровно
    тем, ради чего ADR-0003 и завёл ручной список посадочных.

    Правило одно на проверку ввода владельца в админке и на пересборку
    условий переразметкой: скрипт заводит условия мимо формы, и без
    общего определения дубль уехал бы на витрину молча (тикет 22).
    """
    available = (
        attribute.values.count()
        if attribute.kind == Attribute.Kind.CHOICE
        # У «да/нет» весь справочник — это «да» и «нет»; токены
        # querystring тут ни при чём, считаются значения
        else len(BOOL_LABELS)
    )
    return len(values) >= available
...
class LandingCondition(models.Model):
    """Одно значение атрибута, которым посадочная сужает категорию.
    ...
    Значений одного атрибута у посадочной бывает несколько, и между
    собой они объединяются по ИЛИ — тем же правилом, что и в фильтре
    каталога: «зеркала в раме» — это и алюминий, и багет, а «с
    подсветкой» — контурная, фронтальная и комбинированная разом.
    Сужается посадочная по-прежнему одним-двумя атрибутами (ADR-0003):
    предел кладётся на них, а не на строки.
    """
