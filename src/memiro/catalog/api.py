...
пожелание, и вместо числа эндпоинт зовёт оставить заявку. Так же
отвечает эндпоинт и товару, у которого владелец погасил цену расчёта:
конструктор на карточке остаётся, число не называется ни там, ни здесь
(ADR-0008). Предпосчитанные
...
class PriceQuote(pydantic.BaseModel):
    ...
    Итога может не быть: ни размер за пределом производства, ни товар
    с погашенной ценой расчёта числа не получают — вместо него стоит
    признак «нужна заявка», и в обоих случаях цену назовёт менеджер.
    """
