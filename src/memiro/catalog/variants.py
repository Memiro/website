...
Здесь живут правила сборки, разбор присланного и цена собранного; про
HTTP модуль не знает — отказ уходит `ValidationError` со словами,
обращёнными к владельцу, а показывает его админка. Разбор живёт тут
же, рядом со словами отказа: разъехавшись по файлам, правило
читалось бы в одном месте, а его формулировка в другом.

Цену собранного считает `tariffs.price_of()` — та же функция, которой
пересчёт запишет её варианту после сохранения (`catalog.repricing`).
Вторым расчётом в админке она однажды разошлась бы с записанной, и
владелец добавлял бы вариант, увидев одно число, а в таблице находил
другое.
...
NOT_MEASURED = (
    "Укажите ширину и высоту в миллиметрах — положительными числами, "
    f"каждая до {calculator.MAX_INPUT_SIDE_MM} мм."
)
...
# Чем вариант отличается от товара — и, главное, чего владелец здесь
# не выбирает. Пустой набор флажков тут норма, а не недозаполненность,
# и сказать об этом больше негде: в списке вариантов подписи нет
VALUES_HELP = (
    "Отметьте, чем вариант отличается от товара. Ничего не отмечено — "
    "вариант берёт значения товара целиком. Отмеченное заменяет "
    "умолчание товара, а не добавляется к нему; двух значений одного "
    "атрибута у варианта не бывает — это второй вариант."
)
...
def compose_from(
    *,
    ...
    """То же, но из того, что прислала страница, — строками.

    Разбор стоит рядом с правилами, а не у вьюхи: отказывают они
    одними и теми же словами, и разъехавшись по файлам, правило и его
    формулировка однажды разошлись бы по смыслу.
    """
    return compose(
        product,
        width_mm=_side(width_mm),
        height_mm=_side(height_mm),
        value_ids=_value_ids(value_ids),
        order=_order(order),
    )
...
def _side(raw: str | None) -> int:
    ...
    try:
        ...
        raise ValidationError(NOT_MEASURED) from None


def _order(raw: str | None) -> int:
    """Каким по счёту вариант стоит на карточке.

    Пустое поле — ноль, а не отказ: порядок владелец задаёт не всякому
    варианту, и требовать его значило бы мешать заводить первый.
    А вот отрицательное — отказ, а не молчаливый ноль: поле хранит
    неотрицательное число, и подменив минус нулём, конструктор
    поставил бы вариант не туда, куда владелец целился.
    """
    if raw in (None, ""):
        return 0
    try:
        ...
        raise ValidationError(NOT_ORDERED) from None
    if order < 0:
        raise ValidationError(NOT_ORDERED)
    return order


def _value_ids(raw: Sequence[str]) -> list[int]:
