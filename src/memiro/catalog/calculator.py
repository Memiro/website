...
def is_measurable(width_mm: int, height_mm: int) -> bool:
    """Габариты, которые расчёт вообще берёт в руки.

    Это защита разбора, а не предел производства: тот живёт в
    «Параметрах расчёта» и на предпосчитанные варианты не
    распространяется вовсе. Здесь отсекается разве что число
    в километр.

    Правило одно на всех, кто принимает миллиметры со стороны:
    эндпоинт расчёта у покупателя и конструктор вариантов у
    владельца. Отказывают они разными словами — покупателю и
    владельцу говорят разное, — но берут в руки одно и то же.
    """
    return all(
        1 <= side <= MAX_INPUT_SIDE_MM for side in (width_mm, height_mm)
    )
