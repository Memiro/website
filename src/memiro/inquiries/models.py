...
class Inquiry(models.Model):
    ...
    # а прочитать заявку менеджер должен и через год.
    #
    # Сегодня поле пусто у всякой заявки, которую витрина принимает:
    # калькулятор считает на карточке, а отправлять заявку оттуда
    # нечем с тикета 07. Заполненным оно бывает у заявок, принятых
    # до него, и снова станет — у позиции, а не у заявки (тикет 14,
    # ADR-0009). Единственное объяснение этого поля: `inquiries/api.py`
    # ссылается сюда, а не пересказывает
