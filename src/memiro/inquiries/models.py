...
from memiro.inquiries.limits import MAX_WISH_LENGTH
...
class InquiryItem(models.Model):
    """Позиция заявки — зеркало в её составе снимком (CONTEXT.md).

    Не «товар заявки», как её звала прежняя подпись: товар живёт в
    каталоге, а здесь снимок с ним рядом — своя конфигурация, своё
    название и своя цена на момент обращения. Словарь эти два термина
    развёл намеренно («Товар», _Avoid_: позиция).
    ...
    # Что покупатель сказал об этом зеркале своими словами: второй
    # выключатель, вырез не там, где предусмотрел владелец. Личное
    # пожелание (CONTEXT.md) конфигурацией не является: в расчёт оно
    # не идёт и цену не меняет — тем и ценно, что сайт его посчитать
    # не умеет. У позиции, а не у заявки: у зеркала в ванную и
    # у зеркала в прихожую хотелки разные, и сложенные в один
    # комментарий они заставили бы менеджера разбирать, что к чему
    # относится (ADR-0009). Длина ограничена: свободный текст без
    # потолка описывает не пожелание, а попытку нагрузить журнал.
    # Текст многострочный — покупатель пишет его в `textarea`, а
    # письмо менеджеру печатает абзацы как есть, — потому `TextField`
    # с потолком, а не `CharField`: тип говорит о хранимом правду
    wish = models.TextField(
        "личное пожелание", max_length=MAX_WISH_LENGTH, blank=True
    )
