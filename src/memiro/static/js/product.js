...
(() => {
  ...
  const NOTES = {
    ...
    inquiry:
      "Такой размер мы считаем индивидуально: добавьте зеркало " +
      "в заявку и опишите размер — менеджер назовёт цену.",
    failed:
      "Не удалось посчитать цену. Попробуйте ещё раз или добавьте " +
      "зеркало в заявку и напишите в ней размер.",
  };
  ...
  // Позвать к заявке — там, где цены не будет вовсе. Формы на карточке
  // больше нет (тикет 07), и ведём к подборке, а не к свободной форме:
  // свободная увезла бы менеджеру размер без зеркала, о котором он.
  // Адрес приезжает из шаблона, чтобы маршруты Django не переписывались
  // в браузере
  const invite = (text) => {
    const link = document.createElement("a");
    link.className = "link-underline";
    link.href = root.dataset.calcInquiryUrl;
  ...
  // Настроенное покупателем уезжает в подборку вместе с зеркалом
  // (тикет 14, ADR-0009). Кнопка «Добавить в заявку» живёт в shop.js,
  // а поля калькулятора — здесь: второй их разбор однажды разошёлся
