...
from memiro.adapters.common.inquiry_wording import phone_words, price_words, size_words, specification_words
...
def letter_subject(inquiry: Inquiry) -> str:
    ...
    subject = f"Заявка от {inquiry.name}, {phone_words(inquiry.phone)}"
    if inquiry.items:
...
def letter_html(inquiry: Inquiry) -> str:
    ...
    return "\n".join(
        [
            ...
            f'<table {_TABLE} width="100%" style="background:{_PAPER_2};">',
            '<tr><td align="center" style="padding:24px 12px;">',
            (
                f'<table {_TABLE} width="100%" style="max-width:600px;background:{_PAPER};'
...
def _date_words(moment: datetime) -> str:
    ...
    local = moment.astimezone(STUDIO_TIMEZONE)
...
def _client_block(inquiry: Inquiry) -> list[str]:
    ...
    return [
        ...
        f'<table {_TABLE} style="margin-top:14px;font-size:15px;line-height:24px;">',
        _contact_row("Телефон", f'<a href="tel:{phone}" style="{_LINK}">{html.escape(phone_words(inquiry.phone))}</a>'),
...
def _specification(configuration: InquiryConfiguration) -> list[str]:
    ...
    return [
        ...
        f'<table {_TABLE} width="100%" style="margin-top:12px;font-size:14px;line-height:20px;">',
        *rows,
        "</table>",
    ]
